...
var TreeTimeLine = React.createClass({
  ...
  //     Dates.dates[canvas].push(date.toJSON().slice(0, 10));
  //   }
  // },
  ...
  renderCanvas: function(startDay, endDay, canvas) {
    ...
    for (var i = 0; i < this.state.apiData.length; i++) {
      ...
      if (Dates.dates[canvasProps.canvas].indexOf(this.state.apiData[i].date) !== -1) {
    ...
    //   .domain([new Date(Dates.dates[canvas][Dates.dates[canvas].length - 1]), new Date(Dates.dates[canvas][0])]).nice()
    ...
    function update(root, canvas) {
      ...
      var nodeEnter = node.enter().append('svg:g')
        ...
        .on('mouseenter', function(d) {
          ...
          if (d.depth === 3) {
            helpers.mouseOver(d, component.props.mouseOver);
          }
        })
      ...
      nodeUpdate.select('circle')
          ...
          // })
          .style('fill', function(d) {
            return Images.describeNodeImages(d, defs);
          })
